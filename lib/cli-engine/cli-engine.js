...
const debug = require("debug")("ec0lint:cli-engine");
const validFixTypes = new Set(["directive", "problem", "suggestion", "layout"]);

const co2Module = require("./co2-module")
 ...
 * @property {string} [cacheLocation] The cache file to use instead of .ec0lintcache.
 ...
 * @property {boolean} [useEc0lintrc] False disables looking for .eslintrc
...
/**
 * CO2 module return data.
 * @typedef {Object} CO2Data
 * @property {number} sizeSaved The saved size after applying ec0lint suggestions
 * @property {number} reducedFootprint The amount of reduced carbon footprint after applying ec0lint suggestions
 */
...
/**
 * It will calculate the directory size
 * @param {string} directory Path to directory
 * @returns {number} Size of directory
 * @private
 */
...
class CLIEngine {
    ...
    constructor(providedOptions, { preloadedPlugins } = {}) {
        const options = Object.assign(
            ...
            defaultOptions,
            {
                cwd: process.cwd(), rootSize: getFolderSize(process.cwd(), (err, bytes) => {

                    if (process.env.state === 'TEST') return 1
                    return err ? 0 : bytes / 1024 / 1024
                })
            },
            providedOptions
        );
        ...
        const configArrayFactory = new CascadingConfigArrayFactory({
            ...
            useEc0lintrc: options.useEc0lintrc,
            builtInRules,
            loadRules,
            getEslintRecommendedConfig: () => require("../../conf/ec0lint-recommended.js"),
            getEslintAllConfig: () => require("../../conf/ec0lint-all.js")
        });
    ...
    static getErrorResults(results) {
        ...
        results.forEach(result => {
            ...
            const sumOfSize = result.messages.map(message => co2Module(message).sizeSaved).reduce((acc, curr) =>
                acc + curr, 0)
            const sumOfCO2 = result.messages.map(message => co2Module(message).reducedFootprint).reduce((acc, curr) =>
                acc + curr, 0)

            const percentage = sumOfSize * 100 / 1
            ...
            if (filteredMessages.length > 0) {
                filtered.push({
                    ...
                    fixableWarningCount: 0,
                    co2Module: `${sumOfSize} mB (${percentage.toFixed(2)}%) reduced ${sumOfCO2} g CO2`
                });
            }
        });
    ...
    /**
     * Checks if a given path is ignored by ec0lint.
    ...
    getFormatter(format) {
        ...
        if (typeof resolvedFormatName === "string") {
            ...
            if (!namespace && normalizedFormatName.indexOf("/") > -1) {
                ...
                try {
                    const npmFormat = naming.normalizePackageName(normalizedFormatName, "ec0lint-formatter");
            ...
            try {
                ...
                if (format === "table" || format === "codeframe") {
                    ex.message = `The ${format} formatter is no longer part of core ec0lint. Install it manually with \`npm install -D ec0lint-formatter-${format}\``;
                } else {
