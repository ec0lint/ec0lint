    ...
    constructor(providedOptions, { preloadedPlugins } = {}) {
        ...
        const configArrayFactory = new CascadingConfigArrayFactory({
            ...
            getEslintRecommendedConfig: () =>
                require("../../conf/ec0lint-recommended.js"),
    ...
    static getErrorResults(results) {
        ...
        const rootSize =
            process.env.state === "TEST"
                ? 1
                : getFolderSize(process.cwd(), (err, bytes) => {
        ...
        results.forEach((result) => {
            ...
            const sumOfSize = result.messages
                .map((message) => co2Module(message).sizeSaved)
                .reduce((acc, curr) => acc + curr, 0);
            const sumOfCO2 = result.messages
                .map((message) => co2Module(message).reducedFootprint)
                .reduce((acc, curr) => acc + curr, 0);

            const percentage = (sumOfSize * 100) / rootSize;
            ...
            if (filteredMessages.length > 0) {
                filtered.push({
                    ...
                    fixableWarningCount: 0,
                    co2Module: `${sumOfSize} mB (${percentage.toFixed(
                        2
                    )}%) reduced ${sumOfCO2} g CO2`,
                });
            }
        });
