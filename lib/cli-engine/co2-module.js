...
function co2Module(message) {
    if (message.ruleId === "lighter-http") {
        const module = message.message.match(
            /axios|got|request|make-fetch-happen|superagent|needle|simple-get/
        )[0];
        ...
        const sizeSaved = 2.0;
        // const reducedFootprint = computeCarbonFootprint(sizeSaved)
        const reducedFootprint = ((sizeSaved * 0.81) / 1024) * 442;
        return {
            sizeSaved,
            reducedFootprint,
        };
