...
module.exports = function(results) {
    ...
    results.forEach(result => {
        ...
        if (messages.length > 0) {
            output += `<testsuite package="org.ec0lint" time="0" tests="${messages.length}" errors="${messages.length}" name="${result.filePath}">\n`;
            messages.forEach(message => {
                const type = message.fatal ? "error" : "failure";

                output += `<testcase time="0" name="org.ec0lint.${message.ruleId || "unknown"}" classname="${classname}">`;
            ...
            output += `<testsuite package="org.ec0lint" time="0" tests="1" errors="0" name="${result.filePath}">\n`;
