...
module.exports = {
    meta: {
        ...
        messages: {
            unexpected: "Do not import axios. Instead use fetch."
        },
        schema: [] // no options,
    },
    create(context) {
        ...
        function isHttpModule(name) {
            ...
            return base in ["axios", "got", "request", "make-fetch-happen", "superagent", "needle", "simple-get"];
        }
        ...
        function reportIfMissing(node, name) {
            ...
            if (allowed.indexOf(name) === -1 && isHttpModule(name)) {
                context.report({ node, messageId: "unexpected", data: `Do not import ${name}. Instead use fetch.` });
            }
        }
