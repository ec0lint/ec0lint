...
const fs = require("fs"),
    ...
    { Ec0lint } = require("./ec0lint"),
    CLIOptions = require("./options"),
    log = require("./shared/logging"),
    RuntimeInfo = require("./shared/runtime-info");

const debug = require("debug")("ec0lint:cli");
...
const cli = {
    ...
    async execute(args, text) {
        ...
        if (options.printConfig) {
            ...
            const engine = new Ec0lint(translateOptions(options));
            const fileConfig =
        ...
        if (options.fix) {
            ...
            await Ec0lint.outputFixes(results);
        }
        ...
        if (options.quiet) {
            ...
            resultsToPrint = await Ec0lint.getErrorResults(resultsToPrint);
        }
        ...
        if (await printResults(engine, resultsToPrint, options.format, options.outputFile)) {
            ...
            if (!errorCount && tooManyWarnings) {
                log.error(
                    "ec0lint found too many warnings (maximum: %s).",
                    options.maxWarnings
                );
            }
