    ...
    Object.entries(globalsConfig)
        .forEach(([configuredGlobal, configuredValue]) => {
            try {
                ...
                throw new Error(`ec0lint configuration of global '${configuredGlobal}' in ${source} is invalid:\n${err.message}`);
            }
        });
}
...
function validateProcessor(processorName, source, getProcessor) {
    if (processorName && !getProcessor(processorName)) {
        throw new Error(`ec0lint configuration of processor in '${source}' is invalid: '${processorName}' was not found.`);
    }
}
...
function validateConfigSchema(config, source = null) {
    ...
    if (!validateSchema(config)) {
        throw new Error(`ec0lint configuration in ${source} is invalid:\n${formatErrors(validateSchema.errors)}`);
    }
