...
const
    ...
    merge = require("lodash.merge"),    // ec0lint-disable-line
    ...
    } = require("@ec0lint/ec0lintrc/universal"),
...
const debug = require("debug")("ec0lint:linter");
const MAX_AUTOFIX_PASSES = 10;
const DEFAULT_PARSER_NAME = "espree";
const DEFAULT_ECMA_VERSION = 5;
const commentParser = new ConfigCommentParser();
const DEFAULT_ERROR_LOC = { start: { line: 1, column: 0 }, end: { line: 1, column: 1 } };
const parserSymbol = Symbol.for("ec0lint.RuleTester.parser");
...
function createDisableDirectives(options) {
    ...
    for (const ruleId of directiveRules) {
        ...
        // push to directives, if the rule is defined(including null, e.g. /*ec0lint enable*/)
...
function getDirectiveComments(ast, ruleMapper, warnInlineConfig) {
    ...
    ast.comments.filter(token => token.type !== "Shebang").forEach(comment => {
        ...
        const match = /^(ec0lint(?:-env|-enable|-disable(?:(?:-next)?-line)?)?|exported|globals?)(?:\s|$)/u.exec(directivePart);

        if (!match) {
            return;
        }
        const directiveText = match[1];
        const lineCommentSupported = /^ec0lint-disable-(next-)?line$/u.test(directiveText);
        ...
        if (directiveText === "ec0lint-disable-line" && comment.loc.start.line !== comment.loc.end.line) {
        ...
        switch (directiveText) {
            case "ec0lint-disable":
            case "ec0lint-enable":
            case "ec0lint-disable-next-line":
            case "ec0lint-disable-line": {
                const directiveType = directiveText.slice("ec0lint-".length);
...
const eslintEnvPattern = /\/\*\s*ec0lint-env\s(.+?)(?:\*\/|$)/gsu;
                    ...
                    report(...args) {
                        ...
                        if (problem.suggestions && !(rule.meta && rule.meta.hasSuggestions === true)) {
                            if (rule.meta && rule.meta.docs && typeof rule.meta.docs.suggestion !== "undefined") {
                                ...
                                throw new Error("Rules with suggestions must set the `meta.hasSuggestions` property to `true`. `meta.docs.suggestion` is ignored by ec0lint.");
                            }
...
class Linter {
     ...
     * @param {"flat"|"ec0lintrc"} [config.configType="ec0lint"] the type of config used.
     */
