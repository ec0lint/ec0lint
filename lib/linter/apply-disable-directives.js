...
function createIndividualDirectivesRemoval(directives, commentToken) {
     ...
     *
     *     // ec0lint-disable-line rule-one , rule-two , rule-three -- comment
     *                            ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
     */
    ...
    const unusedDisableDirectives = processed
        .map(({ description, fix, unprocessedDirective }) => {
            ...
            return {
                ruleId: null,
                message: description
                    ? `Unused ec0lint-disable directive (no problems were reported from ${description}).`
                    : "Unused ec0lint-disable directive (no problems were reported).",
